...
public:
    ...
    void Execute(vtkObject *caller, unsigned long eventId, void *) override
    {
        ...
        planeWidget->GetPlane(plane);
        clipper_->SetClipFunction(plane);
    ...
    void setClipper(vtkSmartPointer<vtkClipDataSet> clipper) {
        clipper_ = clipper;
    }
...
private:
    ...
    vtkSmartPointer<vtkClipDataSet> clipper_ = nullptr;
};
...
int main(int, char *[])
{
    ...
    cwCallback2->setClipper(clipper);

    // 创建切块演员
    vtkSmartPointer<vtkDataSetMapper> clipperMapper = vtkSmartPointer<vtkDataSetMapper>::New();
    clipperMapper->SetLookupTable(pColorTableModel);
    clipperMapper->SetInputConnection(clipper->GetOutputPort());
    clipperMapper->SetInterpolateScalarsBeforeMapping(1);
    clipperMapper->SetScalarRange(0, 1); // 最大值最小值
