...
int main(int, char*[])
{
    ...
    std::list<vtkIdType> neighbors{0,1,2,5};
    ...
    grid->DeepCopy(extractSelection->GetOutput());
    // 至此，grid失去了原有的数据结构，因此想多次隐藏单元的话，需要有两份数据：
    // 备份一个原始数据，每次获取到隐藏单元与原始数据进行selection
    // 一份显示数据，屏幕图显变化以显示数据进行展示
    grid->Modified();
    // *************************************** 隐藏结束 ****************************************************
